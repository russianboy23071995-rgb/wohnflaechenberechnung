...
from wohnflaechen.presentation.widgets.home_dashboard_widget import HomeDashboardWidget
...
def _home_icon() -> QIcon:
    """Zeichnet ein einfaches Haus-Symbol für die Navigationsleiste."""
    size = 32
    pixmap = QPixmap(size, size)
    pixmap.fill(Qt.GlobalColor.transparent)
    painter = QPainter(pixmap)
    painter.setRenderHint(QPainter.RenderHint.Antialiasing)
    pen = QPen(QColor("#2e2e2e"), 2.2)
...
class MainWindow(QMainWindow):
    ...
    def _build_menu(self) -> None:
        ...
        self.home_action.setIcon(_home_icon())
        self.home_action.setShortcut(QKeySequence("Ctrl+Home"))
        self.home_action.setToolTip("Zum Dashboard")
    ...
    def _build_nav_bar(self) -> None:
        ...
        nav_bar.setToolButtonStyle(Qt.ToolButtonStyle.ToolButtonIconOnly)
        nav_bar.addAction(self.home_action)
    ...
    def _open_hub_from_dashboard(self, key: str) -> None:
        ...
        if entry.project_id is not None:
            self._open_woflv_from_dashboard(key)
    ...
    def _update_project_state(self) -> None:
        ...
        if has_project and self._current_project:
            ...
            self.preface_editor.set_project(
                self._current_project.id, self._current_project.measurement_on_site
            )
